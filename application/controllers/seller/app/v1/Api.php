    ...
    public function login()
    {
        ...
        */
        ...
        if ($login) {
            ...
            $seller_data = fetch_details(['user_id' => $data[0]['id']], 'seller_data');
    ...
    */
    ...
    public function get_customers()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            $seller_id = $this->input->post('seller_id', true);
            $seller_permit = fetch_details(['user_id' => $seller_id], 'seller_data', 'permissions');
    ...
    public function get_zipcodes()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $search = (isset($_POST['search']) &&  !empty(trim($_POST['search']))) ? $this->input->post('search', true) : '';
            $zipcodes = $this->Area_model->get_zipcodes($search);
    ...
    public function get_attribute_set()
    {
       ...
       */
        ...
        if (!$this->form_validation->run()) {
            ...
            $order = (isset($_POST['order']) && !empty(trim($_POST['order']))) ? $this->input->post('order', true) : 'ASC';
            $search = (isset($_POST['search']) && !empty(trim($_POST['search']))) ? $this->input->post('search', true) : "";
            $result = $this->Attribute_model->get_attribute_set($sort, $order, $search);
            print_r(json_encode($result));
        }
    }
    ...
    public function get_attributes()
    {
       ...
       */
        ...
        if (!$this->form_validation->run()) {
            ...
            $result = $this->Attribute_model->get_attributes($sort, $order, $search, $attribute_set_id);
            print_r(json_encode($result));
        }
    }
    ...
    public function get_attribute_values()
    {
       ...
       */
        ...
        if (!$this->form_validation->run()) {
            ...
            $result = $this->Attribute_model->get_attribute_value($sort, $order, $search, $attribute_id);
            print_r(json_encode($result));
        }
    }
    ...
    public function get_seller_details()
    {
        ...
        */
        ...
        $id = $this->input->post('id', true);
        $data = fetch_details(['id' => $id], 'users');
        $seller_data = fetch_details(['user_id' => $id], 'seller_data');
        $data = array_values(array_merge($data, $seller_data));
        for ($i = 0; $i < count($seller_data); $i++) {
            $seller_data[$i]['logo'] = base_url() . $seller_data[$i]['logo'];
            $seller_data[$i]['national_identity_card'] = base_url() . $seller_data[$i]['national_identity_card'];
    ...
    public function update_user()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            $id = $this->input->post('id', true);
            $seller_data_id = fetch_details(['user_id' => $id], 'seller_data', 'id,address_proof,national_identity_card,logo');
