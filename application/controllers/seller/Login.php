    ...
    public function update_user()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_seller() && ($this->ion_auth->seller_status() == 1 || $this->ion_auth->seller_status() == 0)) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($_POST['edit_seller'])) {
                    // if (!edit_unique($this->input->post('email', true), 'users.email.' . $this->input->post('edit_seller', true) . '') || !edit_unique($this->input->post('mobile', true), 'users.mobile.' . $this->input->post('edit_seller', true) . '')) {
                    //     $response["error"]   = true;
