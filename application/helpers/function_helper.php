...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $seller_id = NULL)
{
    ...
    $discount_filter_data = (isset($filter['discount']) && !empty($filter['discount'])) ? ' pv.*,ceil( if(special_price > 0,( (price-special_price)/price)*100,0)) as cal_discount_percentage, ' : '';
    ...
    $t->db->select($discount_filter_data . ' (select count(id)  from products where products.category_id=c.id ) as total,count(p.id) as sales, p.stock_type ,
     p.is_prices_inclusive_tax, p.type ,GROUP_CONCAT(DISTINCT(pa.attribute_value_ids)) as attr_value_ids,sd.rating as seller_rating,sd.logo as seller_profile, sd.store_name as store_name,sd.store_description, p.seller_id, u.username as seller_name,
     p.id,p.stock,p.name,p.category_id,p.short_description,p.slug,p.description,p.total_allowed_quantity,p.deliverable_type,p.deliverable_zipcodes,p.minimum_order_quantity,
     ...
     p.video_type, p.video, p.tags, p.warranty_period, p.guarantee_period, p.made_in,p.availability,c.name as category_name,tax.percentage as tax_percentage ')
    ...
    if (isset($filter['min_price']) && $filter['min_price'] > 0) {
        $where = ['if( pv.special_price > 0 , pv.special_price , pv.price ) >=' => $filter['min_price']];
    }
    if (isset($filter['max_price']) && $filter['max_price'] > 0 && isset($filter['min_price']) && $filter['min_price'] > 0) {
        $where = ['if( pv.special_price > 0 , pv.special_price , pv.price ) >=' => $filter['min_price']];
        // $where .= " AND if( pv.special_price > 0 , pv.special_price , pv.price ) <= " . $filter['max_price'];
    }
    ...
    if (isset($filter['discount']) && !empty($filter['discount']) && $filter['discount'] != "") {
        ...
        $t->db->group_by('p.id')->having("cal_discount_percentage  <= " . $discount_pr, null, false);
    } else {
        $t->db->group_by('p.id');
    }
    ...
    $discount_filter = (isset($filter['discount']) && !empty($filter['discount'])) ? ' , GROUP_CONCAT( IF( CEIL( IF( special_price > 0, ((price - special_price) / price) * 100, 0 ) ) > ' . $filter['discount'] . ', CEIL( IF( special_price > 0, ((price - special_price) / price) * 100, 0 ) ), 0 ) ) AS cal_discount_percentage ' : '';
    ...
    if (!empty($product)) {
        ...
        for ($i = 0; $i < count($product); $i++) {
            ...
            $product[$i]['seller_rating'] = (isset($product[$i]['seller_rating']) && !empty($product[$i]['seller_rating'])) ? output_escaping($product[$i]['seller_rating']) : 0;
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $delivery_boy_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $seller_id = null)
{
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        $t->db->select('oi.*,p.id as product_id,p.is_cancelable,sd.store_name ,sd.store_description,sd.rating as seller_rating,sd.logo as seller_profile,ot.courier_agency,ot.tracking_id,ot.url,u.username as seller_name,p.is_returnable,p.image,p.name,p.type,oi.status as status,(Select count(id) from order_items where order_id = oi.order_id ) as order_counter ,(Select count(active_status) from order_items where active_status ="cancelled" and order_id = oi.order_id ) as order_cancel_counter , (Select count(active_status) from order_items where active_status ="returned" and order_id = oi.order_id ) as order_return_counter ')
            ->join('product_variants pv', 'pv.id=oi.product_variant_id', 'left')
            ->join('products p', 'pv.product_id=p.id', 'left')
        ...
        if ($order_details[$i]['payment_method'] == "bank_transfer") {
            $bank_transfer = fetch_details(['order_id' => $order_details[$i]['id']], 'order_bank_transfer', 'attachments,id');
        ...
        for ($k = 0; $k < count($order_item_data); $k++) {
            ...
            if (!empty($order_item_data)) {
                ...
                $order_item_data[$k]['store_description'] = (!empty($order_item_data[$k]['store_description'])) ? $order_item_data[$k]['seller_name'] : '';
                $order_item_data[$k]['seller_rating'] = (!empty($order_item_data[$k]['seller_rating'])) ? $order_item_data[$k]['seller_rating'] : "0";
...
function get_seller_invoice_html($order_id, $seller_id)
{
    ...
    if (isset($order_id) && !empty($order_id) && isset($seller_id) && !empty($seller_id)) {
        $s_user_data = fetch_details(['id' => $seller_id], 'users', 'email,mobile,address,country_code');
        $seller_data = fetch_details(['user_id' => $seller_id], 'seller_data', 'store_name,pan_number,tax_name,tax_number');
...
function process_refund($id, $status, $type = 'order_items')
{
    ...
    if ($type == 'order_items') {
        ...
        if (!empty($order_details) && !empty($order_item_details)) {
            ...
            if ($payment_method == 'Bank Transfer' && ($bank_receipt[0]['status'] == "0" || $bank_receipt[0]['status'] == "1")) {
                $wallet_refund = false;
            } else {
                $wallet_refund = true;
            }
...
{
    $res = fetch_details(['id' => $order_item_id], 'order_items', 'otp');
